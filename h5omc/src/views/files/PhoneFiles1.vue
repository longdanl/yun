<template>
	<section style="position: relative;top:-10px">
		<el-form :inline="true" :model="filters" style="margin:6px 0 0 10px">
			<el-form-item>
				<el-button class='buttonCss' @click="handleAddPhone" :disabled="phoneFilesDis"><i class="fa fa-plus-circle" aria-hidden="true" style="color:#8cd41d"></i> 添加</i></el-button></el-button>
			</el-form-item>
			<el-button class='buttonCss' @click="deletePhoneBatch" :disabled="this.sels.length===0||phoneFilesDis"><i class="fa fa-times-circle" aria-hidden="true" style="color:#d13737"></i> 批量删除</el-button>
		...
		<!--彩铃列表-->
		<el-table @sort-change='tableSortChange' @selection-change="selsChange" :header-cell-style="{'background':'#f0f5f7','color':'#000000','padding':'0','border-bottom':'1px solid #cbcbcb'}" :cell-style="excelStyle" :data="phoneList" style="width:695px;margin-left:10px;margin-bottom: 40px;top:-12px;border:1px solid #cbcbcb" element-loading-text="拼命加载中..." v-loading="phoneLoading">
			<el-table-column :selectable='checkboxT' type="selection" width="43">
			</el-table-column>
			<el-table-column prop="is_default" label="默认" width="80" sortable='custom'>
				<template scope="scope">
					<i class="fa fa-check-circle" style="color: green;font-size: 18px" v-show="scope.row.is_default==1"></i>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" width=110 class="showBtn" prop="is_default">
				<template scope="scope">
					<el-button size="small" type="text" :disabled="phoneFilesDis" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
					<el-button type="text" size="small" :disabled="scope.row.is_default==1||phoneFilesDis" @click="deletePhone(scope.$index, scope.row)">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<!--工具条-->
		...
		<!--主叫号码新增界面-->
		<el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false" width="460px" style="position:fixed;left:2%;top:16%">
		...
		<!--主叫号码编辑界面-->
		<el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" width="460px" style="position:fixed;left:2%;top:16%">
...
<script>
import qs from "qs"
import {callingPhone,getRoleName,handleFullScreen} from '../../common/js/util'
...
export default {
	data() {
		return {
			...
			tableKey: [
			{
				...
				name: '音频文件',
				value: 'arbt_file_memo',
				width:'160',
				showTooltip:true
			},{
				name: '视频文件',
				value: 'vrbt_file_memo',
				width:'160',
				showTooltip:true
			}],
			fullscreen:false,
	...
	methods: {
		...
		getRoleName(){
			this.phoneFilesDis = getRoleName(this.phoneFilesDis,"crbt_10086:write")
		},
        ...
        sortByPhone(){
            sortByPhone(this.page,this.limit,this.sortby,this.order).then((res)=>{
                if(this.sortby){
                    if(res.code==0){
